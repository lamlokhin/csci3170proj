...
public class CSCI3170Proj {
    ...
    static void show_popular_parts(Connection conn) throws NumberFormatException, IOException, SQLException {
        ...
        Statement stmt = conn.createStatement();
        ResultSet rs = null;
        rs = stmt.executeQuery("SELECT T.pID, T.pName, T.total_no_trans FROM (SELECT p.pID, p.pName, COUNT(t.tID) AS total_no_trans, ROW_NUMBER() OVER (ORDER BY total_no_trans DESC) AS RowNumber FROM PART p JOIN TRANSACTION t ON p.pID = t.pID GROUP BY p.pID, p.pName ORDER BY total_no_trans DESC) T WHERE T.RowNumber <= " + input + ";");
        // print the parts
